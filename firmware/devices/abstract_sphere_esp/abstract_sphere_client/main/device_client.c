...
static bool map_state_string(const char *state, lighting_state_t *out_state)
{
    if (!state || !out_state) {
        return false;
    }
    ...
    if (strcmp(state, "neutral") == 0) {
        *out_state = LIGHTING_STATE_NEUTRAL;
    } else if (strcmp(state, "low_negative") == 0) {
        *out_state = LIGHTING_STATE_LOW_NEGATIVE;
    } else if (strcmp(state, "low_positive") == 0) {
        *out_state = LIGHTING_STATE_LOW_POSITIVE;
    } else if (strcmp(state, "high_negative") == 0) {
        *out_state = LIGHTING_STATE_HIGH_NEGATIVE;
    } else if (strcmp(state, "high_positive") == 0) {
        *out_state = LIGHTING_STATE_HIGH_POSITIVE;
    } else {
        return false;
    }
...
static void handle_set_state(const cJSON *root)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(lighting_controller_set_state(target_state));
}

static void handle_set_vibration(const cJSON *root)
{
    const cJSON *enabled_item = cJSON_GetObjectItemCaseSensitive(root, "enabled");
    if (!cJSON_IsBool(enabled_item)) {
        ESP_LOGW(TAG, "set_vibration missing boolean enabled");
        return;
    }

    const bool enabled = cJSON_IsTrue(enabled_item);
    ESP_LOGI(TAG, "Command set_vibration -> %s", enabled ? "true" : "false");
    ESP_ERROR_CHECK_WITHOUT_ABORT(motor_controller_set_enabled(enabled));
}

static void handle_set_vibration_level(const cJSON *root)
{
    const cJSON *level_item = cJSON_GetObjectItemCaseSensitive(root, "level");
    if (!cJSON_IsNumber(level_item)) {
        ESP_LOGW(TAG, "set_vibration_level missing numeric level");
        return;
    }

    ESP_LOGI(TAG, "Command set_vibration_level -> %d%%", (int)(level_item->valuedouble * 100.0));
    ESP_ERROR_CHECK_WITHOUT_ABORT(motor_controller_set_level((float)level_item->valuedouble));
}
...
static void handle_command_line(const char *line)
{
    ...
    } else if (strcmp(command->valuestring, "set_vibration") == 0) {
        handle_set_vibration(root);
    } else if (strcmp(command->valuestring, "set_vibration_level") == 0) {
        handle_set_vibration_level(root);
    } else {
        ESP_LOGI(TAG, "Ignoring unsupported abstract_sphere command '%s'", command->valuestring);
    }
...
static void device_client_task(void *arg)
{
    ...
    while (true) {
        ...
        ESP_LOGW(TAG, "Socket disconnected; stopping vibration for safety");
...
{
    ESP_LOGI(TAG, "Start Abstract Sphere TCP client");
