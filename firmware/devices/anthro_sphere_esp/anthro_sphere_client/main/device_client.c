...
static bool map_state_string(const char *state, face_state_t *out_state)
{
    if (!state || !out_state) {
        return false;
    }
    ...
    if (strcmp(state, "neutral") == 0) {
        *out_state = FACE_STATE_NEUTRAL;
    } else if (strcmp(state, "low_negative") == 0) {
        *out_state = FACE_STATE_LOW_NEG;
    } else if (strcmp(state, "low_positive") == 0) {
        *out_state = FACE_STATE_LOW_POS;
    } else if (strcmp(state, "high_negative") == 0) {
        *out_state = FACE_STATE_HIGH_NEG;
    } else if (strcmp(state, "high_positive") == 0) {
        *out_state = FACE_STATE_HIGH_POS;
    } else {
        return false;
    }
...
static void handle_set_state(const cJSON *root)
{
    ...
    ESP_ERROR_CHECK(face_fsm_request_state(target_state));
}
...
static void handle_command_line(const char *line)
{
    ...
    if (strcmp(command->valuestring, "set_state") == 0) {
        ...
        ESP_LOGI(TAG, "Ignoring unsupported anthro_sphere command '%s'", command->valuestring);
    }
...
{
    ESP_LOGI(TAG, "Start Anthro Sphere TCP client");
